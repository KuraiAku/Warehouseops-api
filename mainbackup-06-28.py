...
class ProductCreate(BaseModel): # FOR POST
...
class ProductUpdate(BaseModel): # FOR PUT
    sku: str = Field(min_length=1)
    name: str = Field(min_length=1)
    category: str = Field(min_length=1)
    quantity: int = Field(ge=0)
...
class QuantityUpdate(BaseModel): # FOR PATCH
    quantity: int = Field(ge=0)


class QuantityAdjustment(BaseModel): # FOR PATCH
    change: int
...
def add_product(product: ProductCreate):
    ...
    try:
        ...
        cursor = connection.cursor()
        cursor.execute(""" 
        INSERT INTO products (sku, name, category, quantity, location, reorder_level) 
        VALUES (?, ?, ?, ?, ?, ?)
        """, (
            product.sku, 
            product.name, 
...
def allocated_inventory(order_id: int):
    connection = None
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        ...
        for item in order_items:
            ...
            items_to_allocate.append({
                ...
                "current_allocated": allocated_quantity
            })

        
        for item in items_to_allocate:
            new_allocated_quantity = item["current_allocated"] + item["quantity"]
        
            cursor.execute("UPDATE products SET allocated_quantity = ? WHERE id = ?", (new_allocated_quantity, item["product_id"]))
        ...
        return   {
            "Order allocated successfully": items_to_allocate ,
            "order_id": order_id,
            "status": "allocated"
    }
...
def pick_inventory(order_id: int):
    connection = None
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
...
def update_product_quantity(product_id: int, update: QuantityUpdate):
    ...
    try:
        ...
        if connection:
            connection.rollback()
        raise HTTPException(status_code=500, detail="An error occurred while updating the product quantity")
...
def adjust_product_quantity(product_id: int, update: QuantityAdjustment):
    ...
    try:
        ...
        if connection:
            connection.rollback()
        raise HTTPException(status_code=500, detail="An error occurred while adjusting the product quantity")
